...
uses
  ...
  Web.Types, Web.WebFileDispatcher;
...
{$ENDIF !NEXTGEN}
  ...
  System.Character, System.StrUtils, Web.StringHelper;
...
var
  S: string;
begin
  S := Format('%s=%s; ', [HTTPEncode(AnsiString(FName)), HTTPEncode(AnsiString(FValue))]);
  if Domain <> '' then
...
begin
  ...
  if (NewValue <> '') and (NewValue[StringLow('')] <> '/') then Insert('/', NewValue, 1);
...
begin
  ...
  begin
    if Sp^ in NoConversion then
      ...
      begin
        System.AnsiStrings.AnsiFormatBuf(Rp^, 3, AnsiString('%%%.2x'), 6, [Ord(Sp^)]);
        Inc(Rp,2);
      end;
    Inc(Rp);
    Inc(Sp);
...
begin
  S := DateStr;
  S := StringReplace(S, '-', ':', [rfReplaceAll]); // Change delimiter to work with TParser
...
var
  ...
  begin
    ...
    if StripQuotes then
    begin
      for I := StringHigh(Result) downto StringLow(Result) do
      begin
        LChar := StringChars(Result, I);
        if CharInSet(LChar, ['''', '"']) then
          if InStripQuote and (StripQuoteChar = LChar) then
          begin
            Result := StringRemove(Result, I, 1);
            InStripQuote := False;
          end
