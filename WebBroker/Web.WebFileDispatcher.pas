...
uses
  System.DateUtils, System.StrUtils, Web.WebConst, System.IOUtils, Web.HTTPApp, Web.StringHelper;
...
begin
  I := StringLow(Extensions);
  while I <= StringHigh(Extensions) do
  begin
    if WithDot then
    begin
      Ext[0] := '.';
      ...
      J := 0;
    while (I <= StringHigh(Extensions)) and (Extensions[I] <> ';') do
    begin
...
begin
  ...
  Assert((FProperties.VirtualPath = '/') or not StringEndsWith(FProperties.VirtualPath, '/'));

  LIgnorePath := False;
  LChanged := False;
  LPath := APath;
  if (LPath = '') or (StringChars(LPath, 0) <> '/') then
    StringInsert(LPath, 0, '/');
  // Handle VirtualPath
  if FProperties.VirtualPath <> '/' then
  begin
    if StringStartsWith(LPath, FProperties.VirtualPath, True) then // Ignore case
    begin
      LPath := '/' + StringSubstring(LPath, Length(FProperties.VirtualPath));  // Leave '/'
      LChanged := True;
    end
    else
      LIgnorePath := True;
...
begin
  ...
  begin
      ...
      if S <> '' then
      begin
        if S[StringLow(S)] <> PathDelim then
...
begin
  ...
  begin
    // Normalize path
    if (LPath <> '') and (StringChars(LPath, 0) <> '/') then Insert('/', LPath, 1);
    if Length(LPath) > 1 then
      LPath := StringTrimRight(LPath, ['/']);
  end;
  FVirtualPath := LPath;
end;
